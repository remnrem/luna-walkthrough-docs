...
Instead of using a `bash` shell terminal, you can use the `cmd.exe` to
provide an MD-DOS command line.  Most of the simple Unix shell
commands here have analogs in DOS.  In the cases here, the syntax is basically the same, just
use the alternate command name:

| Command | Linux/macOS | Windows (DOS) |
|---|---|---|
| Copy file(s) | `cp` | `copy` |
| Move file(s) | `mv` | `move` |
| List files | `ls` | `dir` |
...
| View a text file (via a _pager_ app) | `less` | `more` |
| Dump text file to console | `cat` | `type` |
| Show only first _n_ lines of a file | `head` or `head -n 20` |  |
| Sort a file | `sort` | `sort` |
...
Shows amount of RAM and use	mem	free