...
We've [already](../p2/revised.md) handled the issues detected with
`F01`, `F10` and `M10` (gapped EDFs) and `F05` (pre-filtered), and
generated linked-mastoid, resampled versions in the `harm1.lst`
project. Here we'll create a new project (`harm2`), address the remaining
issues, and then perform a final step of epoch-level artifact
detection and interpolation.

!!!info "Alternate approaches to multi-step workflows"
    This approach -- of
    ...
    instead create a new sample list `harm1.lst` that retained the old
    EDFs from `harm1` that would not otherwise change, or use
    symlinks, etc.

## Signals

We'll start by copying all `harm1` EDFs and then update the subset
that need specific changes (from the `harm1` to the `harm2` set).
Assuming you've created the `work/harm2/` folder as required above:
...
Studies with known issues remaining:
 ...
 -  `M04`: dropped (CZ)
 -  _various_: varying levels of artifact based on the [Hjorth signal reviews](../p2/stats.md)

There is not much we can do about those duplicate/flat, or dropped
channels except try to interpolate them, giving the neighboring
channels.  We'll handle that step [below](interp.md).  Here we'll
address the other issues that can be fixed:
...
(e.g. `file1.txt`) for processing steps such as this and use
`include=file1.txt` instead of `id=ID1,ID2,...`

### Adding CZ for `M04`

Finally, CZ does not exist for `M04` but we nonetheless want to
...
of C1, introducing the generic _transform_ command
[`TRANS`](https://zzz.bwh.harvard.edu/luna/ref/evals/#trans).
...
[manipulations](../data.md#annotation-manipulations).  Thus at this point
we'll simply correct these by copying all `v1` (original) annotation
...
We should now have populated `work/harm2/` with 20 EDFs and 20
matching `.annot` files.
...
here](https://zzz.bwh.harvard.edu/luna/vignettes/chep/).

To that end, we'll build a new sample-list, on which we'll base all
parts of this step:
...
Now let's move on to [interpolation](interp.md)