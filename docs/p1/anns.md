...
We also need to handle the arbitrary differences in stage labels
across the different studies use, adopting an approach conceptually
similar to the aliasing done for channel labels.
...
The following extracts the class labels from all 20 individuals
(column 2) and enumerates them: (output ordered by stage)
...
!!!info "Arousals and multiple annotation files"
...
annotation labels.  _(Note: if an alternate annotation itself has a `|`
character, you need to quote (`"`) the entire annotation, as pipes are
used as delimiters in the above.)_
...
command](https://zzz.bwh.harvard.edu/luna/ref/annotations/#write-annots),
...
interpreting it -- more details can be [found here](../prep.md#shell-orientation).)
...
[generate a new sample list for this project](gen.md) next.